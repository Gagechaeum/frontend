  ...
  <div class="font-pretendard min-h-screen bg-white">
    ...
    <div v-if="isLoading" class="flex min-h-screen items-center justify-center">
      <div class="text-center">
        ...
        ></div>
        <p class="text-gray-600">정책 정보를 불러오는 중...</p>
      </div>
    </div>
    ...
    >
      <div class="text-center">
        ...
        <UiButton
          variant="primary"
          @click="() => fetchPolicyDetail(route.params.id)"
        >
          다시 시도
        </UiButton>
      </div>
                ...
                <div class="mb-3 flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p class="text-sm text-gray-600">
                      {{ policyData.supervisingOrganizationName }}
                    </p>
                  </div>
                </div>
              </div>
              ...
              >
                <div class="mb-3 flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p class="text-sm text-gray-600">
                      {{
                        policyData.receivingOrganizationName ||
                        policyData.supervisingOrganizationName
                      }}
                    </p>
                  </div>
              ...
              >
                <div class="mb-3 flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <a
                      :href="policyData.announcementUrl"
                      target="_blank"
            ...
            <div class="space-y-6">
              ...
              <div class="flex items-start">
                ...
                <div class="flex-1">
                  ...
                  <p class="mb-3 text-gray-600">
                    {{ policyData.receivingOrganizationName }} 홈페이지에서
                    온라인으로 신청합니다.
                  </p>
                  <div class="rounded-lg bg-gray-50 p-3">
                    <p class="text-sm text-gray-600">
...
const goToCommunity = () => {
  ...
  router.push({
    ...
    query: {
      ...
      productName: policyData.value.policyName,
    },
  });
};
...
const fetchPolicyDetail = async policyId => {
  try {
    ...
    const response = await getPolicyDetail(policyId);

    // API 응답 데이터를 컴포넌트 데이터 구조에 맞게 변환
    policyData.value = {
      policyId: response.data.policyId,
      policyName: response.data.policyName,
      policySummary: response.data.policySummary,
      policyField: response.data.policyField,
      selectionCriteria: response.data.selectionCriteria,
      supervisingOrganizationName: response.data.supervisingOrganizationName,
      receivingOrganizationName: response.data.receivingOrganizationName,
      noticeDate: response.data.noticeDate,
      modificationDate: response.data.modificationDate,
      applicationPeriod: response.data.applicationPeriod,
      beginDate: response.data.beginDate,
      endDate: response.data.endDate,
      applicationMethod: response.data.applicationMethod,
      contact: response.data.contact,
      supportDetail: response.data.supportDetail,
      supportTarget: response.data.supportTarget,
      announcementUrl: response.data.announcementUrl,
      image:
        response.data.image ||
        'https://readdy.ai/api/search-image?query=government%20policy%20support%20illustration%20with%20official%20documents%20and%20business%20people%20in%20clean%20minimalist%20style%20with%20green%20accent%20colors%20and%20professional%20atmosphere&width=600&height=400&seq=policy001&orientation=landscape',
    };
  } catch (err) {
    console.error('정책 정보 로드 실패:', err);
    error.value = err.message || '정책 정보를 불러오는데 실패했습니다.';
    notification.show('error', '정책 정보를 불러오는데 실패했습니다.');
  } finally {
